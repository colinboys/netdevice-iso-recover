...
> 当前已完整实现的设备类型：**DCGW**（S-RT / E-RT）。当前已支持厂家：**华为、H3C（新华三）**。其他类型（业务 EOR / 管理 EOR / 业务 TOR / 管理 TOR / 防火墙 / CE / 负载均衡 …）已建立占位规则与扩展入口。
...
华为与 H3C 已实现。H3C 当前支持 `display lldp neighbor-information list`。中兴/锐捷为占位，需先补充样例。
...
> `parse_script.py` 已按厂家分发：当前已实现 `huawei` 与 `h3c`；`zte` / `ruijie` 为占位（以 `NotImplementedError` 拒绝），需在 `references/parser-<vendor>.md` 补齐样例后实现对应 `parse_<vendor>()` 函数。CLI 会校验 `--vendor` 取值，避免静默默认。H3C 支持 txt 与 xlsx 脚本，xlsx 会读取第一列命令，并可按 `--device-name` 截取同一工作表中的对应设备脚本块。
>
...
- `parse_lldp.py` — 解析 LLDP 邻居摘要（华为 / H3C 已实现）
- `parse_trunks.py` — 解析聚合口-物理口映射（华为 / H3C 已实现）
...
- `parser-h3c.md` — 华三格式（已实现 DCGW 样例：LLDP / link-aggregation verbose / xlsx 脚本）
...
3. 在三者的 dispatch 中各增加一条分支（`parse_script.py` 已在 `PARSERS` 字典中预留了 zte/ruijie 两个 key，等你把占位的 `raise NotImplementedError` 换成真实实现即可）。
